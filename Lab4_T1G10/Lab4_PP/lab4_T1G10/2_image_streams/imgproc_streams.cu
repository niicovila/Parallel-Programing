...
int main (int argc, char *argv[])
{
   ...
   invert<<<dimGrid,dimBlock,sizeof(double)*B*B,stream1>>>(d_image, d_image_invert, nx, ny);
   cudaMemcpyAsync(h_image_invert,d_image_invert,nx*ny*sizeof(int),cudaMemcpyDeviceToHost,stream1);

   smooth<<<dimGrid,dimBlock,sizeof(double)*B*B,stream2>>>(d_image, d_image_smooth, nx, ny);
   cudaMemcpyAsync(h_image_smooth,d_image_smooth,nx*ny*sizeof(int),cudaMemcpyDeviceToHost,stream2);

   detect<<<dimGrid,dimBlock,sizeof(double)*B*B,stream3>>>(d_image, d_image_detect, nx, ny);
   cudaMemcpyAsync(h_image_detect,d_image_detect,nx*ny*sizeof(int),cudaMemcpyDeviceToHost,stream3);

   enhance<<<dimGrid,dimBlock,sizeof(double)*B*B,stream4>>>(d_image, d_image_enhance, nx, ny);
   cudaMemcpyAsync(h_image_enhance,d_image_enhance,nx*ny*sizeof(int),cudaMemcpyDeviceToHost,stream4);
